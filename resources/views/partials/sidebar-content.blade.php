...
<div
    ...
    <div class="flex items-center gap-3 mb-3">
        <div class="relative overflow-hidden flex items-center justify-center bg-gradient-to-br from-slate-200 to-slate-300 rounded-full shadow-md ring-2 ring-yellow-400/40"
    ...
    <a href="{{ route('profile.index') }}" @click="sidebarOpen = false"
        class="block w-full px-3 py-2 text-center text-xs font-medium bg-slate-500 hover:bg-slate-600 text-white rounded-lg transition-colors duration-200">
...
<div class="space-y-2">
    <a href="{{ route('dashboard.index') }}" @click="sidebarOpen = false"
        ...
        @if (request()->routeIs('dashboard.index'))
            <i class="ri-checkbox-circle-fill ml-auto text-yellow-400 text-sm"></i>
        @endif
    </a>
    ...
    <form action="{{ route('slip-gaji.index') }}" method="get" class="w-full">
        ...
        <button type="submit" @click="sidebarOpen = false"
            class="flex items-center gap-3 px-4 py-3 rounded-lg w-full bg-slate-600/40 border-l-4 border-transparent hover:bg-slate-600/60 text-white transition-all duration-200 group">
            <i
                class="ri-dashboard-line text-xl text-slate-300 group-hover:text-yellow-400 transition-colors duration-200"></i>
    ...
    <a href="https://baca-alquran.sac-po.com" target="_blank" class="block w-full">
        <div
            class="flex items-center gap-3 px-4 py-3 rounded-lg w-full bg-slate-600/40 border-l-4 border-transparent hover:bg-slate-600/60 text-white transition-all duration-200 group">
            <i
                class="ri-git-repository-line text-xl text-slate-300 group-hover:text-yellow-400 transition-colors duration-200"></i>
    ...
    @if (auth()->user()->kerjasama_id == 1)
        <a href="https://sppd-online.sac-po.com/login" target="_blank" class="block w-full">
            <div
                class="flex items-center gap-3 px-4 py-3 rounded-lg w-full bg-slate-600/40 border-l-4 border-transparent hover:bg-slate-600/60 text-white transition-all duration-200 group">
                <i
                    class="ri-newspaper-line text-xl text-slate-300 group-hover:text-yellow-400 transition-colors duration-200"></i>
                <span class="font-medium text-sm">SPPD</span>
            </div>
        </a>
    @endif
    {{-- Divider --}}
    <div class="border-t border-slate-500/30 my-4"></div>
    {{-- Report Error --}}
    <a href="https://report-system-ten.vercel.app/" target="_blank" class="block w-full">
        <div
            class="flex items-center gap-3 px-4 py-3 rounded-lg w-full bg-slate-600/40 border-l-4 border-transparent hover:bg-slate-600/60 text-white transition-all duration-200 group">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                class="text-xl text-slate-300 group-hover:text-yellow-400 transition-colors duration-200 w-5 h-7">
                <path
...
<form action="{{ route('logout') }}" method="post">
    @csrf
    <button type="submit"
        class="w-full flex items-center justify-center gap-2 px-4 py-3 rounded-lg bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white font-semibold shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 active:translate-y-0 transition-all duration-200">
