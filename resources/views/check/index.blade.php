        ...
        <div class="py-10">
            ...
            <div class="flex flex-col items-center mx-2 my-2 sm:justify-center justify-start">
                <div class="overflow-x-auto w-full md:overflow-hidden mx-2 sm:mx-0 sm:w-full">
                    <table
                        class="table table-zebra table-auto w-full table-xs bg-slate-50  sm:table-md text-sm sm:text-md scale-90 md:scale-90">
                                ...
                                @if ($type == 'dikerjakan')
                                    <tr>
                                        <td class="text-center font-semibold" colspan="8">~
                                            ...
                                            {{ $c->updated_at->isoFormat('dddd, D-MMMM-Y') }} ~</td>
                                    </tr>
                                @endif
                                            ...
                                            @else
                                                @forelse($pcp->whereIn('id', $cp) as $i => $pc)
                                                    @if ($pc)
                                                        @php
                                                            $counts = count(
                                                                array_filter($c->pekerjaan_cp_id, function (
                                                                    $value,
                                                                ) use ($pc) {
                                                                    return $value == $pc->id;
                                                                }),
                                            ...
                                            @if (isset($c->approve_status[$j]) && is_array($c->approve_status))
                                                @if ($c->approve_status[$j] == 'accept')
                                                    <div class="flex flex-col justify-center items-center">
                                                        <span
                                                            class="badge bg-emerald-700 px-2 text-xs text-white overflow-hidden">{{ $c->approve_status[$j] }}</span>
                                                        <p>Note: {{ $c->note[$j] }}</p>
                                                    </div>
                                                @elseif($c->approve_status[$j] == 'proccess')
                                                    <div class="flex flex-col justify-center items-center">
                                                        <span
                                                            class="badge bg-amber-500 px-2 text-xs text-white overflow-hidden">{{ $c->approve_status[$j] }}</span>
                                                    </div>
                                                @else
                                                    <div class="flex flex-col justify-center items-center">
                                                        <span
                                                            class="badge bg-red-500 px-2 text-xs text-white overflow-hidden">{{ $c->approve_status[$j] }}</span>
                                                        <p>Note: {{ $c->note[$j] }}</p>
                                                    </div>
                                                @endif
