<x-app-layout>
	<div x-data="{ delOpen: false }" :class="{ 'overflow-hidden': delOpen }" class="bg-slate-500 mx-10 mb-10 shadow-md p-2 rounded-md">
		<div>
		...
		<div class="flex justify-between items-start overflow-hidden mx-10">
			<div class="">
				<form id="filterForm" action="{{ route('users.index') }}" method="GET" class="" style="padding-bottom: 0">
					<select name="filterKerjasama" id="filterKerjasama" class="select select-bordered active:border-none border-none">
						...
						@foreach ($kerjasama as $i)
							<option value="{{ $i->id }}" {{ $filterKerjasama == $i->id ? 'selected' : '' }}>{{ $i?->client?->name }}</option>
						@endforeach
					</select>
					<button type="submit"
                     ...
                     <div class="flex items-start justify-between mb-2">
                         <span class="flex bg-slate-200 rounded-xl shadow p-1">
                            ...
                            <div class="flex justify-end mx-10 gap-2">
                                <button type="submit" class="btn btn-primary font-semibold" id="exportUser" onclick="setExportType('data')">export data</button>
                                <button type="submit" class="btn btn-primary font-semibold" id="exportUser" onclick="setExportType('id_card')">export id card</button>
                                <button type="button" class="btn btn-error font-semibold" id="delUser" @click="delOpen = true">Hapus User</button>
                            </div>
                         </span>
							...
							<tr>
								...
								@elseif(Storage::disk('public')->exists('images/' . $i->image))
									<td class="flex justify-center items-center">
									    <div style="width: 100px; height: 100px; border-radius: 2px;" class="flex justify-center items-center overflow-hidden">
									        <img loading="lazy" src="{{ asset('storage/images/' . $i->image) }}" data-src="{{ asset('storage/images/' . $i->image) }}" alt="" srcset="" style="width: 100%; height: 100%; object-fit: cover; object-position: center; display: block;">
									    </div>
									</td>
								@elseif(Storage::disk('public')->exists('user/' . $i->image))
									<td class="flex justify-center items-center">
									    <div style="width: 100px; height: 100px; border-radius: 2px;" class="flex justify-center items-center overflow-hidden">
									        <img loading="lazy" src="{{ asset('storage/user/' . $i->image) }}" data-src="{{ asset('storage/user/' . $i->image) }}" alt="" srcset="" style="width: 100%; height: 100%; object-fit: cover; object-position: center; display: block;">
									    </div>
									</td>
								@else
								    <td>
								...
								<td class="break-words whitespace-pre-line" style="max-width: 96pt;">{{ $i->nik ? \Illuminate\Support\Facades\Crypt::decryptString($i->nik) : '---' }}</td>
								<td class="break-words whitespace-pre-line" style="max-width: 92pt;">{{ $i->no_hp ? "+62".$i->no_hp : '---' }}</td>
		...
		<div x-show="delOpen" class="fixed inset-0 backdrop-blur-sm bg-black/30 flex items-center justify-center overflow-hidden">
          <div style="width: 60vw; height: 60vh;" class="bg-slate-100 p-4 rounded shadow-lg overflow-hidden">
             <div class="flex items-center gap-4 w-full">
                <p style="width: 95%" class="bg-yellow-500 rounded-md p-2 text-center font-semibold text-slate-800">Konfirmasi Hapus Akun</p>
             ...
             <div class="flex flex-col justify-between overflow-hidden" style="height: 90%;">
                 ...
                 <div class='flex justify-center items-center overflow-y-scroll'>
                     <div id="isiUser" style="min-width: 40vw; max-width: 40vw; min-height: 35vh; max-height: 35vh;" class="bg-white rounded overflow-y-scroll">
                         
                     </div>
                 </div>
                 <div class="flex justify-end overflow-hidden">
                     <button type="submit" id="subUser" onclick="setExportType('delete')" class="btn btn-error">Submit</button>
                 </div>
             </div>
          </div>
        </div>
