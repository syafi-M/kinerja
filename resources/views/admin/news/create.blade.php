<x-app-layout>
	<x-main-div>
		<div class="p-5 py-10">
			<p class="text-center font-bold text-xl sm:text-2xl uppercase">Buat Berita</p>
			<form method="POST" action="{{ route('news.store') }}" class=" my-10" id="form" enctype="multipart/form-data">
				@csrf
				<div class="bg-slate-100 px-10 py-5 rounded shadow">
					...
					<div class="mt-4">
						...
						<input type="date" name="tanggal_lihat" class="input input-bordered w-full" required/>
						<x-input-error :messages="$errors->get('tanggal_lihat')" class="mt-2" />
					</div>
					{{-- tanggal akhir --}}
					<div class="mt-4">
						<x-input-label for="tanggal_tutup" :value="__('Tanggal Berakhir')" />
						<input type="date" name="tanggal_tutup" class="input input-bordered w-full" required/>
