    ...
    <div class="mx-auto w-full max-w-screen-lg space-y-4 px-2 sm:px-3 lg:px-4">
        <section class="px-0.5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    <h1 class="text-2xl font-normal leading-8 tracking-tight text-[var(--md-sys-color-on-surface)]">Tambah Kerjasama</h1>
                    <p class="mt-1 text-sm leading-5 text-[var(--md-sys-color-on-surface-variant)]">Buat data kerjasama baru dan tetapkan pihak approver.</p>
                </div>
        ...
        <form action="{{ route('admin.kerjasama.store') }}" method="POST" class="space-y-4">
            @method('POST')
            @csrf
