<x-app-layout>
    ...
    <div class="bg-slate-500 p-4 mx-36 shadow-md rounded-md">
		...
		<form method="POST" action="{{ route('jabatan.store') }}" class="mx-10 my-10" id="form">
			@csrf
			<div class="bg-slate-100 px-10 py-5 rounded shadow">
				...
				<div class="flex flex-col">
    		    	...
    		    	<div class="flex flex-col">
    					<label for="mitra_id" class="label">Pilih Mitra</label>
        				<span style="height: 14rem;" class="input input-bordered">
            				@forelse($mitra as $i)
                				<div class="flex w-full items-center">
                                    <input type="checkbox" name="kerjasama_id[]" id="mitra_id" class="checkbox checkbox-bordered" value="{{$i->id}}" multiple>
                                    <label for="mitra_id" class="label break-word whitespace-pre-line">{{ $i->client->name}}</label>
                				</div>
            				@empty
        				    @endforelse
        				</span>
