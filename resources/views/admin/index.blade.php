<x-admin-layout>
    ...
    <div class="space-y-6 pb-10">
        <section class="rounded-2xl border border-white/60 bg-white/70 p-5 shadow-sm backdrop-blur sm:p-6">
            ...
            <div class="mt-2 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
                ...
                <a href="{{ route('data-izin.admin') }}" class="inline-flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2 text-sm font-semibold text-blue-700 transition hover:bg-blue-100">
        ...
        <section class="grid gap-4 md:grid-cols-3">
            <article class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    ...
                    <span class="rounded-lg bg-blue-100 p-2 text-blue-600"><i class="ri-notification-3-line"></i></span>
                </div>
            ...
            <article class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    ...
                    <span class="rounded-lg bg-red-100 p-2 text-red-600"><i class="ri-time-line"></i></span>
                </div>
            ...
            <article class="rounded-2xl border border-gray-100 bg-white p-5 shadow-sm">
                <div class="flex items-center justify-between">
                    ...
                    <span class="rounded-lg bg-amber-100 p-2 text-amber-600"><i class="ri-user-line"></i></span>
                </div>
        ...
        <section class="grid gap-4 xl:grid-cols-2">
            <article class="rounded-2xl border border-gray-100 bg-white shadow-sm">
                <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4">
                    ...
                    <span class="rounded-lg bg-red-50 px-2 py-1 text-xs font-semibold text-red-700">{{ $expiringContracts }}</span>
                </div>
                    ...
                    <div class="max-h-80 overflow-y-auto divide-y divide-gray-100">
                        @foreach ($expert as $ex)
                            <div class="flex items-center justify-between gap-3 px-5 py-3">
                                <div class="min-w-0">
                                    <p class="truncate text-sm font-semibold text-gray-900">{{ $ex->client->name }}</p>
            ...
            <article class="rounded-2xl border border-gray-100 bg-white shadow-sm">
                <div class="flex items-center justify-between border-b border-gray-100 px-5 py-4">
                    ...
                    <span class="rounded-lg bg-amber-50 px-2 py-1 text-xs font-semibold text-amber-700">{{ $inactiveUsers }}</span>
                </div>
                ...
                @if ($inactiveUsers > 0)
                    <div class="max-h-80 overflow-y-auto divide-y divide-gray-100">
                        @foreach ($notActiveUsers as $user)
                            <div class="flex items-center justify-between gap-3 px-5 py-3">
                                <div class="min-w-0">
                                    <p class="truncate text-sm font-semibold text-gray-900">{{ $user->name }} | {{ ucwords(strtolower($user->nama_lengkap)) }}</p>
                                    <p class="mt-0.5 text-xs text-gray-500">
                                        Terakhir aktif:
