    ...
    <x-main-div>
		<div class="py-10 px-5">
			...
			<form action="{{ route('qrcode.export')}}" id="exportForm" method="POST">
			    @csrf
				<div class="flex justify-end mx-10 mb-2 gap-x-3">
				    ...
				    <button type="submit" class="bg-yellow-400 px-3 flex gap-x-2 items-center py-2 shadow rounded-md text-2xl">
			...
			<div class="flex justify-center overflow-x-auto mx-10 pb-10">
				<table class="table table-fixed table-zebra w-full shadow-md bg-slate-50" id="searchTable">
					<thead>
						<tr>
						    <th class="bg-slate-300 rounded-tl-2xl">
							...
							<th class="bg-slate-300">QR Code</th>
					...
					<tbody class=" text-sm my-10">
						...
						@forelse ($qr as $i)
							<tr>
							    ...
							    <td>{{ $no++ }}</td>
							    <td> <img src="{{asset('storage/images/'. $i->qr_code )}}"/></td>
