            ...
            <main>
                <div class="px-5 py-5">
                    <form action="{{ route('absensi.update', $absensi->id) }}" method="POST"
                        enctype="multipart/form-data" id="form-absen">
                        @method('PUT')
                        @csrf
                        ...
                        <div class="p-1 rounded my-3 ">
                            ...
                            <span id="labelMap" class="text-white text-center flex flex-col justify-center">
                                ...
                                <p id="resolver">coba refresh browser jika map belum muncul</p>
                            </span>
                            <div id="map"></div>
                        </div>
                        <div class="flex flex-col gap-2">
                            ...
                            <div class="flex flex-col  justify-between">
                                ...
                                <select name="shift_id" id="shift_id" class="select select-bordered font-thin">
                                    ...
                                    @forelse ($shift as $i)
                                        @if (Auth::user()->kerjasama->client_id == $i->client_id && Auth::user()->divisi->jabatan_id == $i->jabatan_id)
                                            @php
                                                $endA = Carbon\Carbon::parse($i->jam_end)
                                                    ->subHour(1)
                                                    ->format('H:i');
                                            @endphp
                                            <option value="{{ $i->id }}"
                                                {{ $absensi->shift->id == $i->id ? 'selected' : '' }}
                                                data-shift="{{ $i->jam_start }}">
                                                {{ $i->jam_start }} - {{ $endA }} |
                                                {{ $i->jabatan->name_jabatan }} |
                                                {{ $i->shift_name }}
                                            </option>
                                        @else
                                        @endif
                                ...
                                @if (Auth::user()->kerjasama->client_id == 1)
                                    <span id="absen-kantor"
                                        data-absen-kantor="{{ Auth::user()->kerjasama->client_id }}" hidden></span>
                                @endif
                            </div>

                            <div>
                                <div>
                                    <label class="required">Perlengkapan: </label>
                                </div>
                                        ...
                                        @forelse ($dev as $arr)
                                            @if (Auth::user()->devisi_id == $arr->id)
                                                @foreach ($arr->perlengkapan as $i)
                                                    <div>
                                                        @php
                                                            $isChecked = in_array($i->name, json_decode($absensi->perlengkapan));
                                                        @endphp
                                                        <input type="checkbox" name="perlengkapan[]" id="perlengkapan"
                                                            {{ $isChecked ? 'checked' : '' }}
                                                            value="{{ $i->name }}"
                                                            class="checkbox checkbox-sm m-2">
                                                        <label
                                                            for="perlengkapan">{{ $i->name }}</label>
                                                    </div>
                                                @endforeach
                                            @else
                                            @endif
                            ...
                            <div>
                                ...
                                <textarea name="deskripsi" id="deskripsi" value="{{ $absensi->deskripsi }}" placeholder="deskripsi..."
                                    class="w-full textarea textarea-bordered">{{ $absensi->deskripsi }}</textarea>
                            </div>
                        ...
                        <div class="flex flex-col justify-center sm:justify-end gap-3 mt-2 mr-2">
                            ...
                            <span class="flex justify-center gap-3">
                                ...
                                <button
                                    class="p-2 my-2 px-4 text-white bg-blue-500 hover:bg-blue-600 rounded transition-all ease-linear .2s"
                                    id="btnAbsen">Update Absen</button>
                        ...
                        if (jadi <= 90) {
                            ...
                            if (kesimH == 0) {
                                $('#labelWaktuStart').html(
                                    `tunggu ${kesimM} menit ${kesimS} detik lagi untuk absen`);
