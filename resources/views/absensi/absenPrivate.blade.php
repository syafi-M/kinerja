...
<body class="font-sans antialiased  bg-slate-400">
	<div class="min-h-screen pb-[12.5rem]">
		...
		<div class="sm:mx-10 mx-5 bg-slate-500 rounded-md shadow-md">
			<main>
				<div class="px-5 py-5">
					<form action="{{ url('absensi-private-store') }}" method="POST" enctype="multipart/form-data" id="form-absen">
						@method('POST')
						@csrf
								...
								@forelse ($jadwal as $jad)
								    ...
								    <label>Jadwal Hari ini: </label>
    									@if (Carbon\Carbon::now()->format('Y-m-d') == $jadiTGL)
    										...
    										@else
    											<span class="input input-bordered" disabled>Tanggal: {{ $jad->tanggal }}, Shift:
    												{{ $jad->shift->shift_name }}, Area: {{ $jad->area->nama_area }}</span>
    											@php
    												$hasJadwal = true;
    											@endphp
    										@endif
						...
						<span class="hidden" id="dataUser" data-userId="{{ $uID }}" lati="{{ $harLok->latitude }}" long="{{ $harLok->longtitude }}" radius="{{ $harLok->radius }}" client="{{ $harLok->client }}"></span>
					</form>
				</div>
			</main>
		</div>
...
// 		var long = document.getElementById('long')
        ...
        var latit = $('#dataUser').attr('lati');
        var long = $('#dataUser').attr('long');
        var radius = $('#dataUser').attr('radius');
        var client = $('#dataUser').attr('client');
...
// 			console.log("Lat ",lati," Long ", longi," Rad ", radi," Client ", client);
