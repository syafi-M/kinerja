<x-app-layout>
    <x-main-div>
        <style>
            ...
            /*penghasilan*/
            .container-data #gaji_pokok, #bpjs {
                top: 51.5%;
                 @media (min-width: 768px) {
                     top: 51.4%; 
                }
            }
            .container-data #gaji_lembur, #pinjaman {
                top: 54.3%;
                 @media (min-width: 768px) {
                     top: 54.2%; 
                }
            }
            .container-data #tj_jabatan, #absen {
                top: 56.8%;
                 @media (min-width: 768px) {
                     top: 56.7%; 
                }
            }
            .container-data #tj_kehadiran, #lain_lain {
                top: 59.3%;
                 @media (min-width: 768px) {
                     top: 59.2%; 
                }
        ...
        <div class="py-10">
            <div>
                <p class="text-center text-lg sm:text-2xl font-bold uppercase">Data Slip Gaji <br> {{ Carbon\Carbon::createFromFormat('Y-m', $bulan)->isoFormat('MMMM Y') }}</p>
            </div>
                ...
                <div class="flex flex-col gap-2 mt-5 bg-slate-200 p-4 drop-shadow-md rounded-md w-fit">
                    ...
                    <div class="flex gap-2 justify-center sm:justify-start overflow-hidden">
                        <form action="" method="get" class="overflow-hidden">
                            <input type="month" name="bulan" value="{{ $bulan }}" id="month" class="input input-sm input-bordered">
                ...
                <div class="flex flex-col items-center mx-2 my-5 sm:justify-center justify-start">
                    <div class="overflow-x-auto md:overflow-hidden mx-2 sm:mx-0 bg-slate-50 p-2 rounded-md" id="divImg">
                        <div class="rounded-md relative" style="user-select: none;">
                            <img src="{{ asset('logo/desain_slip.png') }}" width="450" oncontextmenu="return false;" class="rounded-md slip-img" />
                            ...
                            <div style="z-index: 10;" class="absolute inset-0 container-data">
                                ...
                                @endphp
                                <!--bulan-->
                                <p id="bulan" class="absolute font-semibold" style=" left: 51.5%; ">{{ Carbon\Carbon::createFromFormat('Y-m', $slip->bulan_tahun)->isoFormat('MMMM Y') }}</p>
                                <p id="dibuat" class="absolute" style="right: 5%; font-weight: 500;">Dibuat Pada : {{ Carbon\Carbon::createFromFormat('Y-m-d', $slip->created_at->format('Y-m-d'))->isoFormat('D MMMM Y') }}</p>
                                <!--data diri-->
                                <p id="nama" class="absolute font-semibold capitalize" style=" left: 20%; ">{{ ucwords(strtolower(Auth::user()->nama_lengkap)) }}</p>
                                <p id="jabatan" class="absolute font-semibold capitalize" style=" left: 20%; ">{{ Auth::user()->divisi->jabatan->name_jabatan }}</p>
                                <p id="mitra" class="absolute font-semibold capitalize" style=" left: 20%; ">{{ Auth::user()->kerjasama->client->name }}</p>
                                <p id="status" class="absolute font-semibold capitalize" style=" left: 20%; ">{{ $slip->status ? 'Kontrak' : 'Training' }}</p>
                                <!--penghasilan-->
                                <p id="gaji_pokok" class="absolute font-semibold" style=" left: 25%; ">{{ toRupiah($slip?->gaji_pokok) }}</p>
                                <p id="gaji_lembur" class="absolute font-semibold" style=" left: 25%; ">{{ toRupiah($slip?->gaji_lembur) }}</p>
                                ...
                                <p id="tj_lain" class="absolute font-semibold" style=" left: 25%; ">{{ toRupiah($slip?->tj_lain) }}</p>
                                <p id="total_penghasilan" class="absolute" style=" left: 25%; font-weight: 700; ">{{ toRupiah($totalPenghasilan) }}</p>
                                <!--potongan-->
                                <p id="bpjs" class="absolute font-semibold" style=" left: 77%; ">{{ toRupiah($slip?->bpjs) }}</p>
                                <p id="pinjaman" class="absolute font-semibold" style=" left: 77%; ">{{ toRupiah($slip?->pinjaman) }}</p>
                                <p id="absen" class="absolute font-semibold" style=" left: 77%; ">{{ toRupiah($slip?->absen) }}</p>
                                ...
                                <!--total bersih-->
                                <p id="total_bersih" class="absolute" style=" left: 39%; font-weight: 800; ">{{ toRupiah($totalBersih) }}</p>
                            </div>
                        </div>
