    ...
    <x-main-div>
        <div class="py-10 sm:mx-10">
            ...
            <div class="flex flex-col items-center mx-2 my-2 sm:justify-center justify-start">
                ...
                <form action="{{ route('direksi-cekKontrak') }}" method="GET" class="flex flex-col justify-between gap-2 bg-slate-100 rounded w-full px-2 py-3 mb-5">
                    <div>
                ...
                <div style="overflow-x: auto;" class="w-full">
                    @php
                        $validKontrakIDs = $kontrak->filter(fn($k) => $k->send_to_atasan == 1 && !$k->ttd_atasan && $k->ttd)->pluck('id');
                    @endphp
                    <form 
                        x-data="{
                            selected: [],
                        ...
                        method="POST" 
                        action="{{ route('direksi-accKontrak') }}" 
                        ...
                        <div class="flex justify-end my-5 px-4 gap-2">
                            <template x-if="selectAll || selected.length > 0">
                                <div class="flex gap-2">
                                    <button type="submit" @click="acc = true" class="btn btn-success btn-sm sm:btn-md">Terima Terpilih</button>
                                    <button type="submit" @click="acc = false" class="btn btn-error btn-sm sm:btn-md">Tolak Terpilih</button>
                                </div>
                            </template>
                        </div>
                                    ...
                                    <tr class="text-center">
                                        <th class="p-1 py-2 bg-slate-300 rounded-tl-2xl">
                                            <template x-if="allIds.length > 0">
                                                <input type="checkbox" x-model="selectAll" @change="toggleAll()" class="checkbox checkbox-sm">
                                            </template>
                                        </th>
                                        ...
                                        <tr>
                                            <td class="text-center">
                                                @if($i->send_to_atasan == 1 && !$i->ttd_atasan && $i->ttd)
                                                    <input type="checkbox" :value="{{ $i->id }}" x-model="selected" class="checkbox checkbox-sm">
                                                @endif
                                            </td>
                                            <td>{{ ucwords(strtolower($i->nama_pk_kda)) }}</td>
                                            <td>{{ $i->jabatan_pk_kda }}</td>
                                            <td>{{ $i->status_pk_kda }}</td>
                                            <td class="text-center">{{ $i->tgl_mulai_kontrak }} - {{ $i->tgl_selesai_kontrak }}</td>
                                            ...
                                            <td class="text-center">
                                                <a href="{{ route('form-kontrak-preview', ['id' => $i->id]) }}" class="btn btn-sm btn-info text-white text-lg"><i class="ri-eye-line"></i></a>
                                            </td>
