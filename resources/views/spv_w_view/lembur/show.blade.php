<x-app-layout>
        ...
        <div class="w-full px-3 py-4 mx-auto max-w-7xl sm:px-5 lg:px-6">
            <div class="p-4 mb-4 bg-white border rounded-lg shadow-sm border-white/60 ring-1 ring-slate-900/5">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <a href="{{ route('spvw.overtime-application.create', array_filter(['client_id' => $spvwClientId])) }}"
                        ...
                        <div class="p-4 bg-white border rounded-lg shadow-sm border-slate-200">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                ...
                                @if ($status == 'pending' && !($isSubmissionLocked ?? false))
                                    ...
                                    <a href="{{ route('spvw.overtime-application.edit', array_filter(['id' => $overtime->id, 'client_id' => $spvwClientId])) }}"
            ...
            <div class="flex justify-end w-full my-2">
                <form id="bulkOvertimeForm" action="{{ route('spvw.overtime-bulk.status') }}" method="post"
                    style="display: none;">
                    @csrf
                    @method('PATCH')
                    @if (isset($spvwClientId) && $spvwClientId)
    ...
    <script>
        ...
        async function viewDetail(id) {
            const url = "{{ route('spvw.get-overtime-id', ':id') }}".replace(':id', id);
        ...
        function deleteOvertimeConfirmed() {
            ...
            form.method = 'POST';
            form.action = `/SPVW/overtime-application/${pendingDeleteId}`;
        ...
        function submitOvertimeConfirmed() {
            ...
            const form = document.createElement('form');
            form.method = 'POST';
            form.action = `/SPVW/overtime-change-status/${pendingSubmitId}`;
