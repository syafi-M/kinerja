        ...
        <div class="w-full max-w-3xl px-3 py-4 mx-auto sm:px-5 lg:px-6">
            <div class="p-4 mb-4 bg-white border rounded-lg shadow-sm border-white/60 ring-1 ring-slate-900/5">
                <div class="flex items-center gap-3">
                    <a href="{{ route('spvw.overtime-application.history', array_filter(['client_id' => $spvwClientId])) }}"
                        ...
                        aria-label="Kembali ke riwayat lembur">
                    ...
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 text-sm leading-5 text-slate-500">
                            Perbarui data lembur sebelum pengajuan dikirim.
                        </p>
                    </div>
                </div>
            </div>

            <form action="{{ route('spvw.overtime-application.update', $overtime->id) }}" method="POST" enctype="multipart/form-data"
                x-data="overtimeForm()" class="space-y-4">
                @csrf
                @method('PUT')
                ...
                <section class="p-4 bg-white border rounded-lg shadow-sm border-slate-200 sm:p-5"
                    x-data="imagePreview('{{ !empty($overtime->foto_bukti) ? asset('storage/' . $overtime->foto_bukti) : '' }}', '{{ !empty($overtime->foto_bukti) ? basename($overtime->foto_bukti) : '' }}')">
                        ...
                        <label
                            ...
                            <span class="min-w-0">
                                ...
                                <span class="mt-0.5 block text-xs leading-4 text-slate-500">
                                    Lembur berdasarkan shift kerja.
                                </span>
                            </span>
                        </label>
                ...
                <section class="p-4 bg-white border rounded-lg shadow-sm border-slate-200 sm:p-5">
                    ...
                    <textarea name="desc" id="desc" rows="4" placeholder="Contoh: Menyelesaikan pekerjaan tambahan di area..."
                        ...
                        required>{{ old('desc', $overtime->desc) }}</textarea>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        ...
                        <ul class="mt-2 space-y-1 text-xs leading-5 list-disc list-inside text-slate-600">
                            ...
                            <li>Data yang sudah diajukan tidak bisa diedit dari halaman ini.</li>
                        </ul>
                    </div>
                </div>
            </div>
    ...
    <script>
        function overtimeForm() {
            return {
                ...
                manualType: @js(old('type_overtime_manual', $overtime->type_overtime_manual) ?? ''),
                manualTypeRaw: @js(old('type_overtime_manual', $overtime->type_overtime_manual) ?? ''),
                manualTypeDisplay: '',
