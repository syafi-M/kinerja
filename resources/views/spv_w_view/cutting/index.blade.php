    ...
    <x-main-div>
        <div class="w-full max-w-3xl px-3 py-4 mx-auto sm:px-5 lg:px-6">
                    ...
                    <div class="flex items-center min-w-0 gap-3">
                        ...
                        <div class="min-w-0">
                            ...
                            <p class="mt-1 text-sm leading-5 text-slate-500">
                                Catat potongan performa personil dengan keterangan lengkap.
                            </p>
                        </div>
                    </div>
                    <a href="{{ route('spvw.cutting.history', array_filter(['client_id' => $spvwClientId])) }}"
            ...
            <form id="cuttingForm" x-data="cuttingForm()" enctype="multipart/form-data"
                data-store-url="{{ route('spvw.cutting.store') }}" class="space-y-4">
                ...
                <section class="p-4 bg-white border rounded-lg shadow-sm border-slate-200 sm:p-5">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            <label for="date_cutting" class="mb-1.5 block text-sm font-semibold text-slate-700">
                        ...
                        <div>
                            <label for="type_cutting" class="mb-1.5 block text-sm font-semibold text-slate-700">
                                Tipe Cutting <span class="text-red-500">*</span>
                            </label>
                            <select name="type_cutting" id="type_cutting" x-model="selectedTypeCutting"
                                class="min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-800 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100"
                                required>
                                <option value="">Pilih tipe cutting</option>
                        ...
                        <div x-show="selectedTypeCutting === 'Lainnya'" x-collapse class="sm:col-span-2">
                            <label for="type_cutting_manual" class="mb-1.5 block text-sm font-semibold text-slate-700">
                                Tipe Cutting Manual <span class="text-red-500">*</span>
                            </label>
                            <input type="text" name="type_cutting_manual" id="type_cutting_manual"
                                x-model="manualType" placeholder="Masukkan tipe cutting manual"
                                class="min-h-11 w-full rounded-lg border border-slate-300 bg-white px-3 py-2.5 text-sm text-slate-800 outline-none transition focus:border-sky-500 focus:ring-2 focus:ring-sky-100"
                ...
                $.ajax({
                    ...
                    .fail(xhr => {
                        if (xhr.status === 422 && xhr.responseJSON?.errors) {
                            ...
                            showAlert('error', xhr.responseJSON?.message || 'Terjadi kesalahan saat menyimpan data cutting. Silakan coba lagi.');
                        }
                    });
            }
