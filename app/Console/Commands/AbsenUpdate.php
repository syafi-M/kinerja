            ...
            foreach ($abs as $key) {
                if ($key->user_id == $userId && $key->kerjasama->client_id != 1) {
                    foreach ($shift as $s) {
                        ...
                        if($key->shift->shift_name != 'Shift Malam' && $key->shift->shift_name != 'MALAM' && $key->shift_id == $s->id && $key->absensi_type_pulang == null && Carbon::now()->format('H:m:s') > $key->shift->jam_end && $key->keterangan != 'izin') 
                        {
